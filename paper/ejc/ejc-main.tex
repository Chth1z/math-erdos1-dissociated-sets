...
That proof is non-effective, because it approximates a lattice of rank $d-1$ by a primitive one through a density argument.
...
explicit sets below Bohman's constant. Since $f$ is nonincreasing (double every element and adjoin an odd number), each
...
$f(n)<\varepsilon$. The construction is due to the AI system GPT-6 Astra; we follow the exposition of Bloom
\cite{EP1}, whose notation we adopt. The proof has two parts. The first is an elementary iterative construction of
...
lattices in the space of lattices \cite{HoKa23}, which is non-effective. Bloom remarks that an effective version ``could
prove something like $f(n)\le n^{-c/\log\log n}$ for infinitely many $n$''. We note in passing that, combined with the
...
\end{enumerate}
Its \emph{height} is $h=\langle v,\one_d\rangle$, and $\Gamma=\Lat\oplus\Z v$ satisfies
...
\end{lemma}
\begin{proof}
Let $x=\sum_jm_j\ell_j=Q\lambda+\sum_jm_je_{r_j}$ with $\lambda=\sum_jm_jb_j$. If $\lambda=0$ then all $m_j=0$ and $x=0$.
Otherwise $|\lambda|_\infty\ge1$ by (A1), and by \eqref{eq:coeff}
$|x|_\infty\ge Q|\lambda|_\infty-\sum_j|m_j|\ge(Q-K)|\lambda|_\infty\ge Q-K>0$.
\end{proof}
...
Set $\theta_1=Q/2$ and $\rho_1=\rho$. For $k\ge1$, provided $\theta_k,\rho_k>0$, put
\begin{gather*}
...
\theta_{k+1}=\frac{Q}{2^{k+1}}\,c_1\cdots c_k,\qquad \rho_{k+1}=\rho\,a_1\cdots a_k ,
\end{gather*}
...
The code and certificate data are available at [repository URL].
...
\bibitem{EP1} T.~F.~Bloom. Erd\H{o}s Problem \#1, \url{https://www.erdosproblems.com/1}; includes the proof exposition of the construction of GPT-6 Astra (last edited 3 September 2026). Accessed 4 September 2026.
